...
double Polynomial::binarySearch(double min, double max, bool inc)
{
	double mid = (min + max) / 2;
	double val = getValue(mid);
	if (abs(val) < BIN_PRECISION)
		return mid;
	else if ((val > 0) != inc) 
		return binarySearch(mid, max, inc);
	else
		return binarySearch(min, mid, inc);
}
	...
	else
	{
		...
		if (incAtNegInf == (val1 <= 0))
		{
			...
			while (getValue(endCrit + nextVal) * val1 > 0)
				nextVal *= 2;
			roots.push_back(binarySearch(endCrit + nextVal, endCrit, !incAtNegInf));
		}
		...
		for (int critInd = 0; critInd < critPts.size() - 1; critInd++)
		{
			...
			{//points on opposite sides
				roots.push_back(binarySearch(critPts.at(critInd), critPts.at(critInd + 1), val2 > val1)); 
			}
			val1 = val2;
		}
		...
		if (incAtPlusInf == (val1 <= 0))
		{
			...
			while (getValue(endCrit + nextVal) * val1 > 0)
				nextVal *= 2;
			roots.push_back(binarySearch(endCrit, endCrit + nextVal, incAtPlusInf));
		}
	}
	return roots;
}
